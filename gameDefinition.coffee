...
window.Game = Ember.Namespace.create()
...
Game.Block = Em.Object.extend({
  ...
  wonBy: ( ->
    ...
    winner = Game.arrayWonBy(squares, accessor)
    if winner
...
Game.Board.reopenClass({
  ...
  materialize: ({marks, lastMove}) ->
    ...
    blocks = [0..8].map( (i) ->
      ...
      return Game.Block.materialize({board: board, x: xBlock, y: yBlock},
...
# window.board = Game.Board.materialize(stored)
# Game.ApplicationController = Em.Controller.extend({
#   board: board
# 
